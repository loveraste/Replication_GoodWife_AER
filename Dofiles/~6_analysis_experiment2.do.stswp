...
forval l=1/3{
	...
	* Reshape the data	
	...
	if `l'==3{
		reshape long T00 T10 T01 T02 T11 T12 Tl10 Tl01 Tl02 Tl11 Tl12 Th10 Th01 Th02 Th11 Th12 p10 p01 p02 p11 p12, i(n) j(panel)
		drop n
		reshape long T0 T1 Th0 Tl0 Th1 Tl1 p1 p0, i(panel) j(cost)
		reshape long T Th Tl p, i(panel cost) j(type)
	}
	drop if T==.
	* Define the figure labels
	...
	if `l'==3{
		g stype=type if panel==1 & cost==0
		replace stype=type+3.25 if panel==1 & cost==1
		replace stype=type+6.5 if panel==1 & cost==2
		replace stype=type+9.75 if panel==2 & cost==0
		replace stype=type+13 if panel==2 & cost==1
		replace stype=type+16.25 if panel==2 & cost==2
		g p_y=T+5.6
		g p_x=stype+0.35
		local xlabel `"0.5 "Low" 3.75 `" " " " " "A: Self-Identified Experts" "' 3.75 "Intermediate" 7 "High" 10.25 "Low" 13.5 `" " " " "  "B: Exogenous Types" "' 13.5 "Intermediate" 16.75 "High" "'
	...
	* Export the graphs
	twoway (bar T stype if type==0) ///
		...
		graph export "$graphs/00_Main/Figure_`fignum'_`outcome`l''.png", as(png) replace
		if `l'==3{
		twoway (bar T stype if type==0) ///
			(bar T stype if type==1) ///
			(rcap Th Tl stype, color(black))  ///
