...
local optim_iteration=0
local optim_number_vars=0 
local oob_error=100000
forval i=0(25)400 {
	di "`i'"
	forval numvars=1/`controls_count' {
		cap: rforest qw $controls if qw!=., type(reg) iter(`i') numvars(`numvars')
		cap if `e(OOB_Error)'<`oob_error' {
			cap local oob_error=`e(OOB_Error)'
			local optim_iteration=`i'
			local optim_number_vars=`numvars'	
		}
	}
}
...
save "$cdata/RF_parameters", replace
restore
